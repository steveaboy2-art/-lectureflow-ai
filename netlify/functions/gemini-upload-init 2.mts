...
export default async (req: Request) => {
  ...
  if (!apiKey || !baseUrl) {
    return Response.json({ error: 'Netlify AI Gateway is not available for this project yet.' }, { status: 503 });
  }
  ...
  if (!Number.isFinite(size) || size <= 0) return Response.json({ error: 'Missing audio file size.' }, { status: 400 });
  if (size > 1024 * 1024 * 1024) return Response.json({ error: 'Audio file is too large.' }, { status: 413 });
  ...
  const endpoint = `${baseUrl.replace(/\/$/, '')}/upload/v1beta/files`;
  ...
  if (!upstream.ok) {
    ...
    return Response.json({ error: 'Could not start the Gemini audio upload.', detail }, { status: 502 });
  }

  const uploadUrl = upstream.headers.get('x-goog-upload-url');
  if (!uploadUrl) return Response.json({ error: 'Gemini did not return an upload URL.' }, { status: 502 });
