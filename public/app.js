...
function loadState(){
  ...
  const s = {lectures:[], streak:0, processedTotal:0, settings:{aiProvider:'Gemini via Netlify AI Gateway', driveFolder:'MBBS/Lecture Recordings', notesFolder:'MBBS/Lecture Notes'}};
...
function renderUpload(){
  ...
  </div><div class="section"><div class="panel"><h3>What LectureFlow creates</h3><div class="bar-list"><div class="bar-line"><span>Full notes</span><div class="bar-track"><span style="width:100%"></span></div><b>✓</b></div><div class="bar-line"><span>Revision sheet</span><div class="bar-track"><span style="width:100%"></span></div><b>✓</b></div><div class="bar-line"><span>Recall + viva</span><div class="bar-track"><span style="width:100%"></span></div><b>✓</b></div><div class="bar-line"><span>5 MCQs</span><div class="bar-track"><span style="width:100%"></span></div><b>✓</b></div><div class="bar-line"><span>Transcript</span><div class="bar-track"><span style="width:100%"></span></div><b>✓</b></div></div><p class="gateway-note">Powered by Gemini through Netlify AI Gateway. Your audio goes to Gemini for processing; LectureFlow stores the generated study notes locally on this device.</p></div></div></div>`;
...
async function pollInteraction(id){
  ...
  while(Date.now()-started < 15*60*1000){
    ...
    const secs=Math.round((Date.now()-started)/1000);el('processMessage').textContent=`Gemini is listening to the lecture and organizing it… ${secs}s`;
    await sleep(3000);
  }
...
async function processLectureReal(){
  ...
  try{
    ...
    if(!health.ok)throw new Error('Gemini is not active on this Netlify project yet. Make sure Netlify AI Features are enabled.');
...
function renderSettings(){
  el('view-settings').innerHTML=`<div class="settings-list"><div class="settings-card"><h3>AI processing</h3><p>LectureFlow uses Gemini through Netlify AI Gateway. No Gemini API key is stored in your browser.</p><div class="integration-row"><div class="integration-name"><div class="integration-icon">G</div><div><strong>Gemini 3.8 Flash</strong><small>Audio → transcript-grounded MBBS notes</small></div></div><button class="connection-pill" id="geminiInfo">Checking…</button></div><div class="integration-row"><div class="integration-name"><div class="integration-icon">N</div><div><strong>Netlify AI Gateway</strong><small>Secure model access and usage billing</small></div></div><span class="connection-pill connected">Connected</span></div></div>
