...
function renderSettings(){
  ...
  <div class="settings-card" id="googleDriveCard"><h3>Google Drive cloud backup</h3><p>Connect your Google account and LectureFlow will save each completed lecture as a Google Doc in your Drive.</p><div class="integration-row"><div class="integration-name"><div class="integration-icon">G</div><div><strong id="driveStatus">Google Drive</strong><small id="driveStatusText">Not connected</small></div></div><button class="primary-btn" id="connectDriveBtn" style="width:auto;margin:0">Connect Google Drive</button></div></div>
...
function renderLectureTab(l,tab){
  const c=el('lectureTabContent'), lectureOnlyNotes=l.lectureOnlyNotes||[], fullNotes=l.fullNotes||[], mustKnow=l.mustKnow||[], professor=l.professor||[], questions=l.questions||[], viva=l.viva||[], mcqs=l.mcqs||[], confusing=l.confusingAreas||[], readMore=l.topicsToReadMore||[];
  if(tab==='lecture') c.innerHTML=`<div class="note-intro"><strong>What was actually taught:</strong> This section contains only the lecturer's content, cleaned for readability. Textbook additions and AI explanations are kept out.</div>`+(lectureOnlyNotes.length?lectureOnlyNotes.map(n=>`<div class="note-section"><h3>${escapeHtml(n.h)}</h3><p>${escapeHtml(n.p)}</p></div>`).join(''):(l.transcript?`<div class="note-section"><h3>Lecture-derived notes</h3><div class="transcript-text">${escapeHtml(l.transcript).replace(/\\n/g,'<br>')}</div></div>`:'<div class="empty">No lecture-only notes were generated for this lecture. The cleaned transcript is shown only as a fallback.</div>'))+`<div class="lecture-source-note">Source rule: if something is not here, do not assume the lecturer taught it. Use Main Notes for MBBS-context explanations and essential textbook links.</div>`;
